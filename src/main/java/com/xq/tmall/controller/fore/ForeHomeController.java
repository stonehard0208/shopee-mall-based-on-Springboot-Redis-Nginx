...
import com.xq.tmall.redis.RedisUtils;
...
public class ForeHomeController extends BaseController {
    ...
    public String goToPage(HttpSession session, Map<String, Object> map) {
       ...
       for(Category category : categoryList){
        ...
        System.out.println(specialProductList);
        map.put("specialProductList", specialProductList);

        //畅销商品list包装
        Set<Object> hotProductIdSet = redisUtils.zrevrange(goods_rank,0,9);//0-9表示score排名前10的商品ID
        map.put("hotProductIdSet",hotProductIdSet);
        //获取畅销品名称
        List<String> hotProductName = new ArrayList<>();
        for(Object o : hotProductIdSet){
            if(o instanceof Integer){
                Product product = productService.get(Integer.valueOf(o.toString()));
                hotProductName.add(product.getProduct_name().length()>9?product.getProduct_name().substring(0,9)+"...":product.getProduct_name());
            }
        }
        map.put("hotProductName",hotProductName);
