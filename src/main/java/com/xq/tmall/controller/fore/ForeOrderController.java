...
public class ForeOrderController extends BaseController {
    ...
    public String orderPay(HttpSession session, @PathVariable("order_code") String order_code) {
        ...
        if (order.getProductOrderItemList().size() == 1) {
            ...
            //更新产品销量的同时，还要进行排行榜对比。
            if(redisUtils.exists(goods_rank)){ //这个zset里面存放商品id吧
                Double ret = redisUtils.zIncr(goods_rank,product.getProduct_id(),productOrderItem.getProductOrderItem_number());
                if(ret<=0){
                    //
                }
            }else{
                boolean t=redisUtils.zAdd(goods_rank,product.getProduct_id(),productOrderItem.getProductOrderItem_number());
                if(t!=true){
                    //
                }
            }
